...
typedef struct {                                /**< typ pro sadu ukazatelů na funkce ("tabulku virtuálních funkcí") */
    int (*cmp)(void *d1, void *d2);             /**< Porovnání velikosti 2 instancí */
    unsigned (*hash)(void *data);               /**< Výpočet hash hodnoty pro danou instanci */
    char * (*to_string)(void *data, char * str);/**< Převod na string. Výsledek se uloží do parametru str */
    void (*from_string)(char * str, void *data);/**< Převod ze stringu na data */
    char * (*get_type)();						/**< Vrátí řetěz, obsahující typ objektu  */
    void (*destruct)(void *data);				/**< Destrukce objektu = korektní uvolnění všech zdrojů */
} Data_Functions_t;
 ...
 * Funkce v tabulce virtuálních funkcí mohou být volány přes základní datový void,
 * ale přitom mohou být definovány pro každý odvozený typ zvlášť.
 * Díky tomu je možné uložit (ukazatele na) instance void i všechny odvozené typy
 * do jedné datové struktury.
 */
typedef struct {
 ...
 * @warning     Oba parametry musí být stejným typem! Nelze porovnávat různé odvozeniny
 *              typu void.
 * @param       d1      Ukazatel na první položku
 * @param       d2      Ukazatel na druhou položku
 * @return      Funkce vrací záporné číslo pokud je první položka menší než druhá,
 *              nulu pokud se rovnají a kladné číslo pokud je první větší než druhá.
...
/**
 * @brief       Funkce Data_To_String vytvoří řetězec z dat objektu a uloží ho do připraveného
 *              ukazatele.
...
/**
 * @brief       Funkce Data_Get_Type vrací řetězec popisující datový typ objektu.
...
/**
 * @brief       Korektně uvolní veškeré zdroje (např. alokovanou dynamickou paměť, otevřené soubory) objektu.
 * @param       data    Ukazatel na objekt.
 */
