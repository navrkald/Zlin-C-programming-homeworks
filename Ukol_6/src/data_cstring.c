...
void data_cstring_from_string(char * str, void *data) {
    Data_Cstring_t* tmpPtr = myMalloc(sizeof(Data_Cstring_t*));
    strcpy(tmpPtr->cstring,str);
		data = tmpPtr;
}
...
void Data_Cstring_Init(Data_Cstring_t * data, char * str) {
    ...
    if (str != NULL) {
        strncpy(data->cstring, str, 255);		// trim the string to 255 char, d.cstring has only 256 bytes
