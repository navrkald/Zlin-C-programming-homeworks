 ...
 * @author  Tomas Dulik
 * @date    9/3/2015
 * @brief   Implementation of Data_Person_t specialization of Data_t abstract type
 */
...
int data_person_cmp(void *d1, void *d2) {
    ...
    if (scmp!=0) {
        ...
        if (acmp!=0) return acmp;				// you can continue with other comparison criteria..
        else return 0;
    }
}
...
unsigned data_person_hash(void *data) {
    ...
    char * ptr = p->name;
    char * end=(char*)	&(p->height)+sizeof(p->height); // points right behind the end of the data structure
    unsigned result = 0;
...
void data_person_from_string(char * str, void *data) {
	Data_Person_t * person = myMalloc(sizeof(Data_Person_t*));
...
void Data_Person_Init(Data_Person_t * data, char * str, double age, double weight, double height) {
    assert(data != NULL);
    data->fncs = &data_person_fncs;
    if (str != NULL) {
        strncpy(data->name, str, 255);		// trim the string to 255 char, d.cstring has only 256 bytes
        data->name[255]='\0';				// if trimmed, insert the terminating 0.
    } else {
        data->name[0] = '\0';
    }
    data->age = age;
...
Data_Person_t * Data_Person_New(char * str, double age, double weight, double height) {
